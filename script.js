  ...
  upgrades.forEach(u => {
    ...
    if (canBuy) {
      ...
      div.onclick = () => {
        ...
        } else if (u.infinite) u.level = (u.level || 0) + 1;
...
function playRandomMusic() {
  ...
  currentMusic.volume = savedVol ? savedVol / 100 : 0.25;
...
async function init() {
  ...
  // Load settings
  const savedVol = localStorage.getItem("volume");
  if (savedVol) { volumeSlider.value = savedVol; volumeLabel.innerText = `${savedVol}%`; }

  const savedMute = localStorage.getItem("mute");
  if (savedMute) { muteToggle.checked = savedMute === "true"; isMuted = muteToggle.checked; }

  const savedTheme = localStorage.getItem("theme");
  if (savedTheme) { themeSelect.value = savedTheme; document.body.dataset.theme = savedTheme; }
